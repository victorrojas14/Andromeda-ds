...
export interface PermisoNativoProps extends HTMLAttributes<HTMLDivElement> {
  ...
  fixed?: boolean
  onClose?: () => void
  /** "Permitir" / "Permitir mientras se visita el sitio" */
  onAllow?: () => void
  /** "Permitir esta vez" (ubicación y micrófono) */
  onAllowThisTime?: () => void
  /** "Bloquear" / "No permitir nunca" */
  onBlock?: () => void
}
...
/** Pin de ubicación nativo (vector 14×16 del set de Figma) */
const PinIcon = () => (
...
/** Micrófono nativo (vector 11×16 del set de Figma) */
const MicIcon = () => (
 ...
 * Componente de apoyo para representar en los flujos la interacción
 * real del usuario con el navegador: card de 318px estilo Chrome
 * (Roboto y chips nativos #d3e3fd). Se muestra/oculta con `open`
 * (por ejemplo, desde un botón) y con `fixed` se ancla arriba a la
 * izquierda como el prompt real.
 */
export function PermisoNativo({
  type = 'notificaciones',
  site = 'www.invex.com',
  requestText,
  open = true,
  ...
  const texto = requestText ?? REQUEST_TEXT[type]

  return (
    <div
      className={[
        'and-permiso',
        fixed && 'and-permiso--fixed',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
      ...
      {...rest}
    >
      <div className="and-permiso__header">
        <span className="and-permiso__site">
          <span className="and-permiso__site-nombre">{site}</span>
          <span className="and-permiso__site-quiere">quiere</span>
        </span>
        <button
          type="button"
          className="and-permiso__close"
          aria-label="Cerrar"
          onClick={onClose}
        >
      ...
      <div className="and-permiso__request">
        {type === 'notificaciones' && <Icon name="bell-outline" size={16} />}
        {type === 'ubicacion' && <PinIcon />}
        {type === 'microfono' && <MicIcon />}
        <span>{texto}</span>
      </div>
      {type === 'notificaciones' ? (
        <div className="and-permiso__actions and-permiso__actions--inline">
          <button type="button" className="and-permiso__chip" onClick={onAllow}>
            Permitir
          </button>
          <button type="button" className="and-permiso__chip" onClick={onBlock}>
            Bloquear
          </button>
        </div>
      ) : (
        <div className="and-permiso__actions">
          <button type="button" className="and-permiso__chip" onClick={onAllow}>
            Permitir mientras se visita el sitio
          </button>
          <button
            type="button"
            className="and-permiso__chip"
            onClick={onAllowThisTime}
          >
            Permitir esta vez
          </button>
          <button type="button" className="and-permiso__chip" onClick={onBlock}>
